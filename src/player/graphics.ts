...
export async function probeGraphics(input = process.stdin, output = process.stdout): Promise<boolean> {
  ...
  return new Promise(resolve => {
    ...
    const finish = (supported: boolean) => {
      ...
      const rest = buffer.replace(/\x1b_G[^\x1b]*(?:\x1b\\|$)/g, "").replace(/\x1b\[6;\d+;\d+t/g, "");
    ...
    const onData = (data: Buffer | string) => {
      ...
      const ratio = cell ? Number(cell[2]) / Number(cell[1]) : 0;
      if (buffer.includes(`${ESC}_Gi=${ID};OK${ESC}\\`) && ratio >= 0.1 && ratio <= 2) {
        cellAspect = ratio; finish(true);
      } else if (new RegExp(`\\x1b_Gi=${ID};(?!OK)[^\\x1b]+\\x1b\\\\`).test(buffer)) finish(false);
    };
    const timer = setTimeout(() => finish(false), 700);
    input.on("data", onData); input.resume(); output.write(graphicsQuery);
  });
}
...
export class GraphicsPainter {
  ...
  clear(): void {
    if (this.uploaded) this.write(deleteImage);
...
export function enableGraphics(): void {
  graphicsPainter = new GraphicsPainter(s => { process.stdout.write(s); });
}
