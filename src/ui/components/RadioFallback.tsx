...
export function RadioFallback({ live, animate, rows, palette, simple = false }: {
  live: boolean; animate: boolean; rows: number; palette: PlayerPalette; simple?: boolean;
}) {
  ...
  return <Box flexDirection="column" alignItems="center" justifyContent="center" height={rows}>
    ...
    {rows >= 8 || (rows >= 3 && rows < 7) ? <Text color={palette.muted}>{animate ? "Decorative animation" : live ? "Station display" : simple ? "Disc display" : "Cover unavailable"}</Text> : null}
  </Box>;
}
