...
describe("isolated spectrum prototype", () => {
  it("enables Linux and Mac by default with an explicit off switch", () => {
    expect(visualizerEnabled({}, "linux")).toBe(true);
    expect(visualizerEnabled({ JUKEBOXCLI_VISUALIZER: "0" }, "linux")).toBe(false);
    expect(visualizerEnabled({}, "darwin")).toBe(true);
    expect(visualizerEnabled({ JUKEBOXCLI_VISUALIZER: "0" }, "darwin")).toBe(false);
    expect(visualizerEnabled({}, "win32")).toBe(false);
